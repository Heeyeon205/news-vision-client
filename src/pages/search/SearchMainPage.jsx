...
export default function SearchMainPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4">
      ...
      {/* 인기 검색어 */}
      <div className="mt-6">
        <h4 className="text-sm font-semibold text-gray-700 mb-2">인기 검색어</h4>
        {words.length === 0 ? (
          <p className="text-sm text-gray-400">검색 키워드가 없습니다.</p>
        ) : (
          <div className="flex flex-wrap gap-2">
            {words.map((word) => (
              <span
                key={word.id}
                className="px-3 py-1 bg-orange-100 text-orange-700 text-sm rounded-full cursor-pointer hover:bg-orange-200"
