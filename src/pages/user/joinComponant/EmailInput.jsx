...
export default function MatchesEmailCode({email, setEmail, emailCode, setEmailCode, setValidationState}) {
  ...
  const authBtn = async () => {
    ...
    if(!valid) { setMsg('이메일 형식에 따라 입력해주세요.'); setColor('red'); return }
    if(email === '') { return }
    try{
  ...
  const authCheckBtn = async () => {
    const { valid } = validateEmail(email);
    if(!valid) { setCheckMsg('이메일 인증을 먼저해주세요'); setCheckColor('red'); return }
    if(emailCode === '') { return; }
    try{
  ...
  return (
    ...
    <label>이메일</label><br/>
      <input
        ...
        />
      <button onClick={authBtn}>인증하기 </button><br/>
